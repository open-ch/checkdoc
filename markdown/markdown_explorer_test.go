...
func TestExtractAllLinks(t *testing.T) {
	...
	assert.Equal(t, 10, len(links), "Expected 10 links. Is the Autolink extension enabled?")
	...
	assert.Equal(t, "nested/relative", string(links[4].Destination))
	assert.Equal(t, "/nested/absolute", string(links[5].Destination))
	assert.Equal(t, "../sibling", string(links[6].Destination))
	assert.Equal(t, "./sub-dir", string(links[7].Destination))
	assert.Equal(t, "mailto:julien@sqooba.io", string(links[8].Destination))
	assert.Equal(t, "#anchor-id", string(links[9].Destination))
}
